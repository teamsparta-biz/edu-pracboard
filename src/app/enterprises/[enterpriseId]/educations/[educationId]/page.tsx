        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 border-2 border-dashed border-border rounded-2xl">
            ...
            {!q && (
              ...
              >
                <Plus className="w-4 h-4" /> 첫 회차 만들기
              </button>
            )}
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map((r) => {
              const lessonCount = getLessonsByRound(r.id).length;
              return (
                <Link
                  key={r.id}
                  href={`/enterprises/${enterpriseId}/educations/${educationId}/rounds/${r.id}`}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="font-semibold text-base truncate">{r.title}</h2>
                    <p className="text-sm text-muted-foreground truncate">{r.description}</p>
                  </div>
                  <span className="hidden sm:inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Layers className="w-3.5 h-3.5" /> 차시 {lessonCount}
                  </span>
