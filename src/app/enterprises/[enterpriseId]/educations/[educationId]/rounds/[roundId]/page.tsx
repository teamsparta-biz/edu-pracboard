...
export default function Lessons() {
  const { enterpriseId, educationId, roundId } = useParams<{
    enterpriseId: string;
    educationId: string;
    roundId: string;
  }>();
        ...
        <div className="mt-6 mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">
              {round.order}회차 · {round.title}
            </h1>
            <p className="mt-2 text-muted-foreground">{round.description}</p>
          </div>
          <button
          ...
          <div className="space-y-3">
            {filtered.map((l) => {
              ...
              return (
                <Link
                  key={l.id}
                  href={`/enterprises/${enterpriseId}/educations/${educationId}/rounds/${roundId}/lessons/${l.id}`}
